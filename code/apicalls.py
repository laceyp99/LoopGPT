...
-------------------------------------<<<<<<<<<<<<<******TESTING******>>>>>>>>>>>>>------------------------------------------------
'''
# to test using this file, comment out the import statement above and use the import below
# import objects, apicalls

def prompt_translation_in_conversation(prompt, messages, melody=False):
    ...
    messages = [
        {
            ...
            Here is the conversation context so far. Please provide a detailed musical description based on the user's prompt in regards to the prior musical generations: {messages}
            """
        }
    ]
    # if melody:
    ...
    translation = completion.choices[0].message.content
    print(translation)
    # print(f"\n\n${calc_price(completion)}\n\n")
    cost = calc_price(completion, mini=True)
    return translation, cost

def generate_finetuned_chords(prompt, model='gpt-4o-2024-08-06', temp=0.0):
...
def generate_chords_in_conversation(messages, temp=0.0):
    """ Generates a 4 bar MIDI chord progression based on the key, mode, and keywords provided.
    ...
    for i in range(4):
        ...
        try:
            ...
            print(f"Validation error: {e}")
    messages.append({"role": "assistant", "content": f"{bars}"})
    return bars, messages, cost

if __name__ == "__main__":
    
    # # Construct the full prompt
    # prompt = 'a sad rhythmic love song'
    # # Generate the fully fleshed out prompt
    # prompt, prompt_cost = apicalls.prompt_translation(prompt=prompt, melody=False)    
    # cp_gen, msgs, cp_cost = apicalls.generate_chords(prompt, 0.0)
    # prompt, cost = prompt_translation_in_conversation(input("Enter in your revisional prompt: "), messages=msgs, melody=False)
    # # print(prompt)
    # msgs.append({"role": "user", "content": prompt})
    # cp_gen, msgs, cp_cost = generate_chords_in_conversation(messages=msgs, temp=0.0)
    pass