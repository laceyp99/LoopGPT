...
with gr.Blocks(css=""".center-title { text-align: center; font-size: 3em; }""") as demo:
    ...
    with gr.Tab(label="Text to MIDI"):
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                temp_input = gr.Slider(
                    0.0, 1.0, step=0.1, value=0.1, label="Temperature (t)"
                )
        ...
        provider_input.change(
            update_model_choices, inputs=provider_input, outputs=model_choice_input
        )
        ...
        model_choice_input.change(
            update_effort_visibility, inputs=model_choice_input, outputs=effort_input
        )
    ...
    with gr.Tab(label="Prompt Editor"):
        ...
        save_button.click(
            save_prompts, inputs=[loop_gen_input, pt_input], outputs=[save_status]
        )
